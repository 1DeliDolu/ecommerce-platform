...
if [ -f "$ROOT_DIR/secrets/jwt_private_key.pem" ]; then
  ...
  if [ "$PERMS" = "600" ]; then
    ...
    fail "jwt_private_key.pem permissions are $PERMS (expected 600)"
  fi
else
...
if command -v docker >/dev/null 2>&1; then
  ...
  docker run --rm \
    ...
    -v "$ROOT_DIR/.dep-check-data:/usr/share/dependency-check/data" \
    owasp/dependency-check:latest \
    --scan /src \
    --format TEXT \
    --failOnCVSS 8 \
    --out /tmp/dep-check-report 2>/dev/null && ok "Dependency scan passed (CVSS < 8)" \
    || fail "Dependency scan found HIGH/CRITICAL CVEs — check /tmp/dep-check-report"
else
